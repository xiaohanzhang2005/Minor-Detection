# 模块说明：
# - Mode B 的命令行入口。
# - 启动共享 loop engine，并接入 direct runner。
...
from src.skill_loop import DirectRunnerConfig, DirectSkillRunner, SkillAgentLoop, SkillAgentLoopConfig
from src.utils.path_utils import normalize_project_paths


def main() -> None:
    parser = argparse.ArgumentParser(description="Run the direct skill iteration loop on val.jsonl without Agent mediation.")
    ...
    parser.add_argument("--workspace-root", default=str(ROOT_DIR / "reports" / "skill_direct_loops"))
    parser.add_argument(
    ...
    result = SkillAgentLoop(config=config, runner=runner).run()
