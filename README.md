...
**完整治理链路，而不是单点年龄猜测**  
Minor Detection 关注的不是一次性分类结果，而是从触发判断、深度识别、证据链输出，到未成年人模式切换、人工复核与风险运营的完整闭环。

**多维证据融合，而不是只看单轮文本**  
系统同时利用单会话、多会话、时间线索、长期画像与 RAG 相似案例，处理真实场景里常见的隐性校园信号与组合证据。

**可持续进化，而不是规则写死上线即冻结**  
能力优化围绕固定数据集持续进行，通过评测、诊断、优化、版本对比与人审门禁逐轮收敛，而不是依赖人工经验零散改规则。

**工程可接入，而不是停留在研究原型**  
项目提供 Skill、工作台、运行时桥接和版本化迭代链路，适合直接嵌入已有文本对话产品，而不要求先建立实名体系或平台级账户控制。

从产品与工程视角看，这带来三个直接收益：

- **更容易接入现有业务**：可直接接入已有文本对话流，接入成本更低
- **更容易解释与复核**：不仅输出标签，还输出画像、证据链、风险等级与下一步建议
- **更容易持续提升**：优化过程以真实边界样本与人审门禁为核心，更适合长期维护
...
| 资源 | 入口 | 说明 |
| --- | --- | --- |
| 社交对话数据集 | <https://huggingface.co/datasets/xiao2005/minor-detection-social-subset> | 面向社交/心理场景的公开子集 |
| 知识对话数据集 | <https://huggingface.co/datasets/xiao2005/minor-detection-knowledge-subset> | 面向知识/教育场景的公开子集 |
| ClawHub Skill | <https://clawhub.ai/xiaohanzhang2005/minor-detection> | 可直接调用的轻量化能力形态 |
| 项目演示视频 | <https://www.bilibili.com/video/BV1MRXYBgEQk/?spm_id_from=333.1387.homepage.video_card.click> | 完整系统演示视频 |
